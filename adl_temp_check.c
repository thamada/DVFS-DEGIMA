...
int main(int argc, char *argv[]) {
  ...
  if (ADL_OK == status_adl ) {
    ...
    printf("\tEngine clock (min,max,step): (%d,%d,%d)\n",lp_adlod_parameters->sEngineClock.iMin, lp_adlod_parameters->sEngineClock.iMax, lp_adlod_parameters->sEngineClock.iStep);
    printf("\tMemory clock (min,max,step): (%d,%d,%d)\n",lp_adlod_parameters->sMemoryClock.iMin,lp_adlod_parameters->sMemoryClock.iMax,lp_adlod_parameters->sMemoryClock.iStep);
    printf("\tCore voltage (min,max,step): (%d,%d,%d)\n",lp_adlod_parameters->sVddc.iMin,lp_adlod_parameters->sVddc.iMax,lp_adlod_parameters->sVddc.iStep);
  ...
  if (ADL_OK == status_adl) {
    ...
    printf("Can't retrieve adapter %d default performance levels. Error code: %d\n", query_adapter_number, status_adl);
  }
